    ...
    let rotationInterval = setInterval(async () => {
      try {
        const data = await fetch(
          `http://192.168.99.106:8080/nearby+fleet+point+${LodzCoordinates.lat}+${LodzCoordinates.lng}+120000`
        );
  ...
  return (
    ...
    >
      ...
      <Marker position={LodzCoordinates}>
        <Popup>Start position for all points</Popup>
      </Marker>
    </MapContainer>
  );
};
